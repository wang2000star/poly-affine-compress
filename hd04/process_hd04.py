...
def main():
    ...
    print("预处理 XOR 模式...")
    text = open(f'{out_dir}/hd04.txt').read()
    text = re.sub(
    ...
    lines1 = []
    lines1.append("=" * 70)
    lines1.append("  策略 1（共享变换）: hd04.txt")
    lines1.append("=" * 70)
    lines1.append("")
    ...
    lines2 = []
    lines2.append("=" * 70)
    lines2.append("  策略 2（各自变换）: hd04.txt")
    lines2.append("=" * 70)
    lines2.append("")
    ...
    # Write files
    with open(f"{out_dir}/hd04_opt1.poly", 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines1))
    print(f"\n  → hd04/hd04_opt1.poly")

    with open(f"{out_dir}/hd04_opt2.poly", 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines2))
    print(f"  → hd04/hd04_opt2.poly")
