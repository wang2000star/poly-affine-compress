...
6. **联合搜索（多输出联合优化）**：当前 pipeline 是各输出独立搜索最佳变换后合并去重。这种贪婪策略可能遗漏全局最优——输出 A 选择 $z_a$（T=5）时，输出 B 被迫用 $z_b$（T=5），合并后 union_T=10；但如果输出 A 接受 $z_a$（T=6）、输出 B 也接受 $z_a$（T=6，本不最佳），合并后共享 $z_a$，union_T=6 反而更小。联合搜索需解决组合爆炸：在 per-output 候选列表中搜索组合 $(\text{cand}_{a,i}, \text{cand}_{b,j}, \dots)$ 使合并后的 union_T 最小。对 k 输出、每输出 t 个候选，搜索空间为 $t^k$，需用启发式（模拟退火、遗传算法）或分支定界剪枝。
