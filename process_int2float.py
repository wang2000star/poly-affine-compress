"""
int2float.txt → ANF → 优化
11 输入 7 输出（int2float 电路）
"""
import numpy as np
import time, os, re
...
def main():
    ...
    for comp in range(1 << n):
        ...
        for f in func_list:
            cur.append(f.substitute_affine(np.eye(n), bc, verify=False))
        u = compute_union_T(cur)
        if u < best_union:
            best_union, best_comp = u, comp
    print(f"  最佳 complement: {best_comp:011b}, union T: {orig_union} → {best_union}")
    ...
    lines1 = []
    lines1.append("=" * 70)
    lines1.append("  策略 1（共享变换）: int2float.txt")
    lines1.append("=" * 70)
    lines1.append("")
    ...
    lines1.append("")
    lines1.append("  简化过程:")
    lines1.append(f"    最佳 complement: {best_comp:011b}")
    ...
    lines2 = []
    lines2.append("=" * 70)
    lines2.append("  策略 2（各自变换）: int2float.txt")
    lines2.append("=" * 70)
    lines2.append("")
